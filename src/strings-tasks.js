...
function sumOfCodes(str) {
  if (!str) return 0;
  return str.split('').reduce((prev, curr) => prev + curr.charCodeAt(), 0);
}
...
function countVowels(str) {
  return str.split('').filter((char) => 'aeyuoiAEYUOI'.includes(char)).length;
}
...
function isPalindrome(str) {
  const init = str
    ...
    .join('');
  const reversed = init.split('').reverse().join('');
